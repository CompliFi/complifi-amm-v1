...
module.exports = async (done) => {
    ...
    const leveragePrimary = bn(0.999996478162223).times(BONE);
    const leverageComplement = bn(1.00000352185018).times(BONE);
    ...
    try {
        ...
        await wait(
            await factory.newPool(vaultAddress, web3.utils.keccak256('x5Repricer'), 0, 0, 0)
        );
       ...
       await collateralToken.methods.approve(vaultAddress, MAX).send({ from: CONTROLLER });
        ...
        await primaryToken.methods.approve(poolContract.address, MAX).send({ from: CONTROLLER });
